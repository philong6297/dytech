 ...
 * the Singleton
 * Logger class
 * for the logging purpose across Turtle
 * system
 */
...
namespace longlp {
  ...
  /* simple printing to stdout logging strategy, caller should ensure
   *
   * thread-safe access */
  ...
  /* simple printing to a file logging strategy, caller should ensure
   *
   * thread-safe access */
 ...
 not guaranteed to be output in the stamped
...
*/
  Logger::Log::Log(LogLevel log_level, const std::string& log_msg) noexcept {
    ...
    std::ostringstream stream;
    stream
      << std::put_time(&tm, "[%d %b %Y %H:%M:%S]")
      << log_level_names[static_cast<int>(log_level)] << log_msg << std::endl;
 ...
 * potentially notify
   ...
   * * the backend worker to swap and flush
 ...
 */
  void Logger::PushLog(Logger::Log&& log) {
    ...
    bool should_notify = false;
    {
      ...
      if ((now - last_flush_) > REFRESH_THRESHOLD ||
          queue_.size() > COUNT_THRESHOLD) {
        should_notify = true;
      }
    }
 ...
 */
  void Logger::LogWriting() {
    ...
    while (true) {
      std::unique_lock<std::mutex> lock(mtx_);
      cv_.wait(lock, [this]() {
        return done_ || queue_.size() > COUNT_THRESHOLD ||
               GetCurrentTime() - last_flush_ > REFRESH_THRESHOLD;
      });
