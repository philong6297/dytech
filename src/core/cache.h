 ...
 * 2023
 *
 * This is a header file implementing the LRU
 * Cache
 */
...
namespace longlp {
  ...
  /**
 * An concurrent LRU cache to reduce load on server disk I/O and improve


   * * * the
 * responsiveness It uses doubly-linked list and hashmap to
   * achieve

   * * O(1) time
 * seek, insert, update nodes that are closer to
   * the header
   * are
   * to be victims to
 * be evicted next time, i.e with
   * older timestamp
   * nodes
   * newly-added or accessed
 * are closer to
   * the tail, i.e. with
   * newer
   * timestamp
 */
   ...
   * It represents a single file
     ...
     * * * cached in the form of an unsigned char vector
   * and serves as a

     * * node
     * in the doubly-linked list data structure
   */
    class CacheNode {
      friend class Cache;
 ...
 *
     ...
     * * destination buffer and return true
 ...
 */
    ...
    /**
   * Given the resource_url and content, try to insert it into the
 ...
 * *
   ...
   * failure reason
     ...
     * * * could be that the content is too big or identical
   * resource_url
