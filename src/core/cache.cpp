...
namespace longlp {
  ...
  auto Cache::CacheNode::Size() const noexcept -> size_t {
    return data_.size();
  }
  ...
  auto Cache::GetOccupancy() const noexcept -> size_t {
    return occupancy_;
  }

  auto Cache::GetCapacity() const noexcept -> size_t {
    return capacity_;
  }
  ...
  void
  Cache::AppendToListTail(const std::shared_ptr<CacheNode>& node) noexcept {
    auto* node_ptr   = node.get();
