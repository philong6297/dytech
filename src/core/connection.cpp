 ...
 * implementing the TCP connection, which
...
namespace longlp {
  ...
  void
  Connection::SetCallback(const std::function<void(Connection*)>& callback) {
  ...
  void Connection::SetLooper(Looper* looper) noexcept {
    owner_looper_ = looper;
  }

  auto Connection::GetLooper() noexcept -> Looper* {
