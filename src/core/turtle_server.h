 ...
 * 1 2023
 *
 * This is a header-file-only class for the
 *
 * Turtle web server
 * setup
 */
 ...
 *
   ...
   * *
   * The rest is already taken care of and in most cases
   * users don't need
   * to
   * touch
 * upon
 ...
 * OnAccept(): Given the
   ...
   * the
   * Poller tells us there is
 * new
   * incoming client connection basic
   * step of
   * socket accept and build

   * * connection and add into the Poller
   * are already
   * taken care of in
   * the
 * Acceptor::BaseAcceptCallback. This
   * OnAccept()
   *
   * functionality is appended to
 * that base
   * BaseAcceptCallback and
   * called
   ...
   public:
    Server(
      ...
      acceptor_ = std::make_unique<
        Acceptor>(listener_.get(), raw_reactors, server_address);
    }

    virtual ~Server() = default;

    /* Not Edge trigger */
    auto OnAccept(std::function<void(Connection*)> on_accept) -> Server& {
