 ...
 * Dec 24 2022
 *
 * This is an implementation file implementing the char Buffer
 * to push-in and
...
namespace longlp {
  ...
  void Buffer::Append(const std::string& new_str_data) {
    Append(
      reinterpret_cast<const unsigned char*>(new_str_data.c_str()),
      new_str_data.size());
  }
  ...
  void
  Buffer::AppendHead(const unsigned char* new_char_data, size_t data_size) {
  ...
  auto Buffer::Data() noexcept -> const unsigned char* {
    return buf_.data();
  }
