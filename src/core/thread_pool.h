 ...
 * 26 2022
 *
 * This is a header file implementing the ThreadPool for
...
namespace longlp {
  ...
  /**
 * This ThreadPool manages the thread resources and acts as the executor


   * * * for
 * client requests upon submitting a task, it gives back a future
   * to
   * be
   * waited
 * for
 * */
  ...
  auto ThreadPool::SubmitTask(F&& new_task, Args&&... args) -> decltype(auto) {
    ...
    auto packaged_new_task = std::make_shared<
