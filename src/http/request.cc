...
auto Request::ShouldClose() const noexcept -> bool {
  return should_close_;
}

auto Request::IsValid() const noexcept -> bool {
  return is_valid_;
}

auto Request::GetMethod() const noexcept -> Method {
  return method_;
}

auto Request::GetVersion() const noexcept -> Version {
  return version_;
}
...
auto Request::ParseRequestLine(const std::string& request_line) -> bool {
  auto tokens = Split(request_line, kSpace);
  if (tokens.size() != 3) {
  ...
  resource_url_ =
    (tokens[1].empty() || tokens[1].at(tokens[1].size() - 1) == '/')
      ? tokens[1] + kDefaultRoute
      : tokens[1];
  return true;
}

void Request::ScanHeader(const Header& header) {
  /* currently only scan for whether the connection should be closed
   * after
